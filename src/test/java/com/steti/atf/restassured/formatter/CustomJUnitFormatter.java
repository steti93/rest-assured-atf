...
public final class CustomJUnitFormatter implements EventListener {
    ...
    private void addOutputToHookMap(String text) {
        if (!currentTestCaseLogs.containsKey("output")) {
            currentTestCaseLogs.put("output", new ArrayList<String>());
        }
        ((List<String>) currentTestCaseLogs.get("output")).add(text);
    }
    ...
    private void handleTestRunFinished(TestRunFinished event) {
        try {
            ...
            rootElement.setAttribute("name", CustomJUnitFormatter.class.getName());
    ...
    final class TestCase {
        ...
        private void addStepAndResultListing(StringBuilder sb) {
            for (int i = 0; i < steps.size(); i++) {
                ...
                sb.append(steps.get(i).getStep().getText());
                do {
                    sb.append(".");
